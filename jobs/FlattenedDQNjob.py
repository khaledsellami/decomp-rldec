...
from rldec.environments.flattened import FlattenedEnv
...
if __name__ == "__main__":
    ...
    n_RS_iterations = 15
    param_grid = dict(
        features_name=[("structural", "interactions"), ("structural", "calls"),
                       ("semantic", "word_count")],
    ...
    data_format = "parquet"
    # param_grid = dict(
    #     features_name=["structural_data/interactions.npy", "structural_data/class_calls.npy",
    #                    "semantic_data/class_word_intersections.npy"],
    #     exclude_outliers=[True, False],
    #     model=[[512, 1024, 1024], [512, 1024, 1024, 512], [1024, 512, 256, 512, 1024]],
    #     lr=[0.0001, 0.0005, 0.001, 0.005],
    #     train_batch_size=[128, 256, 512],
    #     target_network_update_freq=[500, 1000, 1500],
    #     double_q=[True, False],
    #     n_step=[1, 2, 3],
    #     dueling=[True, False],
    # )
    ...
    try:
        ...
        APP = app_name
        features_name = job_params["features_name"][0]
        force_features = f'{granularity}_{job_params["features_name"][1]}.{data_format}'
        APP_PATH = os.path.join(APP_DATA_PATH, APP.lower())
        OUTPUT_PATH = os.path.join(os.path.curdir, "logs", APP.lower())
        EXPERIMENT_NAME = "FlattenedDQN" + str(job_id)
        ...
        eval_config = dict(
            ...
            eval_freq=-1,
            record_last=True,
            interaction_data_path=data_handler.get_path(f"{granularity}_interactions.{data_format}"), #os.path.join(APP_PATH, "structural_data", "interactions.npy"),
            semantic_data_path=data_handler.get_path(f"{granularity}_word_count.{data_format}"), #os.path.join(APP_PATH, "semantic_data", "class_tfidf.npy"),
            metrics=ALLOWED_METRICS,
        )
        # setup global training hyper-parameters
        N_ITERATIONS = 100
        trainer_config["log_level"] = "INFO"
        trainer_config["framework"] = "torch"#"tf2"
